...
export default async function ViewTrafficPage({
  searchParams,
}: {
  ...
  const funnelSteps = [
    { label: "Page Views", count: totalViews, note: "total kunjungan", color: "#3b82f6" },
    { label: "Unique Sessions", count: uniqueSessions, note: "pengunjung unik", color: "#06b6d4" },
    { label: "Scroll 75%", count: totalScroll, note: "baca sampai bawah", color: "#a78bfa" },
    { label: "Lihat Kemitraan", count: kemitraanSessions, note: "sampai section CTA", color: "#fb923c" },
    { label: "Konversi (WA / Reseller)", count: convertedSessions, note: "klik tombol utama", color: "#4ade80" },
  ];

  return (
    <div className="min-h-screen text-white" style={{ background: "var(--bg-900, #09090b)" }}>
      {/* Header */}
      <div
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-xl font-black" style={{ color: "var(--anana-red, #e11b22)" }}>
            anana
          </span>
        ...
        <div className="flex items-center gap-1 flex-wrap">
          ...
          <Link
            href={`/viewtraffic?range=${range}&_=${Date.now()}`}
            className="ml-1 px-3 py-1 rounded-lg text-xs border transition-colors"
            style={{ color: "var(--text-muted)", borderColor: "var(--card-stroke)" }}
          >
            ↻
          </Link>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Page Views", value: totalViews, sub: "total kunjungan", color: "#3b82f6" },
            { label: "Sessions", value: uniqueSessions, sub: "pengunjung unik", color: "#06b6d4" },
            { label: "WA Clicks", value: totalWA, sub: `${pct(totalWA, totalViews)} dari views`, color: "#4ade80" },
            { label: "Reseller Clicks", value: totalKemitraan, sub: `${pct(totalKemitraan, totalViews)} dari views`, color: "#fb923c" },
          ].map((c) => (
            <div
              key={c.label}
              className="rounded-2xl p-5"
              style={{ background: "var(--bg-800, #18181b)", border: "1px solid var(--card-stroke, #27272a)" }}
            >
              <p className="text-xs uppercase tracking-wider mb-2" style={{ color: "var(--text-muted)" }}>
                {c.label}
              </p>
              <p className="text-3xl font-black mb-1" style={{ color: c.color }}>
        ...
        >
          ...
          <span className="text-2xl font-black" style={{ color: "#4ade80" }}>{convRate}%</span>
        </div>
          ...
          >
            ...
            <div className="space-y-4">
              {funnelSteps.map((step) => {
                const width = totalViews > 0 ? Math.max((step.count / totalViews) * 100, 0) : 0;
                return (
                  <div key={step.label}>
                    ...
                    <div className="h-2 rounded-full overflow-hidden" style={{ background: "var(--bg-700, #27272a)" }}>
                      <div
                        className="h-full rounded-full"
                        style={{ width: `${width}%`, backgroundColor: step.color, transition: "width 0.6s ease" }}
                      />
                    </div>
                    <p className="text-[10px] mt-1" style={{ color: "var(--text-muted)" }}>{step.note}</p>
                  </div>
                );
              })}
            </div>
              ...
              <div className="flex items-end gap-2 h-36">
                {daily.map((d) => (
                  <div key={d.date} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
                    <span className="text-[10px] font-bold" style={{ color: "var(--text-muted)" }}>{d.count}</span>
                    <div
                      className="w-full rounded-t min-h-[4px]"
                      style={{
                        height: `${(d.count / maxDaily) * 80}%`,
                        background: "linear-gradient(to top, var(--accent-blue, #2d9cff), var(--accent-cyan, #19c8ff))",
                      }}
                    />
                    <span className="text-[10px]" style={{ color: "var(--text-muted)" }}>{d.date}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {waSources.length > 0 && (
          ...
          >
            ...
            <div className="space-y-3">
              {waSources.map((s) => (
                <div key={String(s.source)} className="flex items-center gap-3">
                  <span className="text-xs w-28 shrink-0" style={{ color: "var(--text-muted)" }}>
                    {s.source ?? "unknown"}
                  </span>
                  <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: "var(--bg-700, #27272a)" }}>
                    <div
                      className="h-full rounded-full"
                      style={{ width: `${(s._count._all / maxWA) * 100}%`, background: "#4ade80" }}
                    />
                  </div>
                  <span className="text-xs font-bold w-6 text-right">{s._count._all}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        >
          ...
          {recentEvents.length === 0 ? (
            <p className="text-sm" style={{ color: "var(--text-muted)" }}>Belum ada event yang tercatat.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="uppercase tracking-wider" style={{ color: "var(--text-muted)", borderBottom: "1px solid var(--card-stroke, #27272a)" }}>
                ...
                <tbody>
                  {recentEvents.map((e) => (
                    <tr key={e.id} style={{ borderBottom: "1px solid var(--card-stroke, #27272a)", color: "var(--text-muted)" }}>
                      <td className="py-2 pr-4 whitespace-nowrap">{relTime(e.createdAt)}</td>
                      <td className="py-2 pr-4">
                        <span className="font-semibold" style={{ color: EVENT_COLORS[e.name] ?? "var(--text-muted)" }}>
                          {e.name}
                        </span>
                      </td>
                      <td className="py-2 pr-4">{e.source ?? "—"}</td>
                      <td className="py-2 font-mono" style={{ fontSize: "10px" }}>
        ...
        <p className="text-center text-xs pb-8" style={{ color: "var(--text-muted)" }}>
          Updated {new Date().toLocaleString("id-ID")} · Range: {rangeLabels[range]}
        </p>
      </div>
    </div>
  );
